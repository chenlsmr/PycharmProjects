"""Python爬虫爬取百度音乐歌曲整体思路如下：
1.搜索：通过搜索界面输入歌手名字找到歌手的歌曲信息。千千音乐：网址
2.找到歌曲信息：通过遍历歌曲列表信息界面获取每个歌曲的ID，以及歌曲的总数
3.下载歌曲：分析网页结构，找到歌曲的下载地址，利用download_music 函数下载歌曲
4.保存歌曲：创建每个歌手单独的文件夹，将爬取的歌曲放在里面。
"""
import requests
...
def save_music(music_name,music_url,artist):
    ...
    try:
        ...
        file = "E:Python/百度音乐/" +artist+'/' + music_name + ".mp3"
